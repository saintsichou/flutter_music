...
class NewSongs {
  ...
  @override
  String toString() {
    return 'NewSongs{albumData:$albumData,columnData:$columnData,concertData:$concertData,contentId:$contentId,linkData:$linkData,mvData:$mvData,singerData:$singerData,songData:$songData,txtData:$txtData,vrbtData:$vrbtData}';
  }
}
...
class Cr {
  ...
  factory Cr.fromJson(Map<String, dynamic> json) => Cr(
        ...
        validateDate: json["validateDate"] == null
            ? null
            : DateTime.parse(json["validateDate"]),
      );

  Map<String, dynamic> toJson() => {
        "copyrihtId": copyrihtId,
        "invalidateDate":
            "${invalidateDate.year.toString().padLeft(4, '0')}-${invalidateDate.month.toString().padLeft(2, '0')}-${invalidateDate.day.toString().padLeft(2, '0')}",
        "productId": productId,
        "resourceIdList": List<dynamic>.from(resourceIdList.map((x) => x)),
        "validateDate": validateDate == null
            ? null
            : "${validateDate.year.toString().padLeft(4, '0')}-${validateDate.month.toString().padLeft(2, '0')}-${validateDate.day.toString().padLeft(2, '0')}",
      };
}
